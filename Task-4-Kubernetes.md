...
Provision a Kubernetes cluster on AWS. Deploy Dockerized application on Kubernetes.
...
Create a pod that uses the above created image. Use the pod.yaml file.
```
vi pod.yaml
```
Add the given content, by pressing `INSERT`
...
spec:
  containers:
  - name: flask-app
    image: <username>/my-flask-demo:tag		##change this image name(DockerHUb Account)
    ports:
      - containerPort: 5000
```
